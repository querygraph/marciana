...
|---|---|---|
| TypeSec and TypeDID | The cognition authority, exact governed-source scope, manifest-only reauthorization, prepared-commit and proposal-free recovery, versioned receipt evidence, and non-disclosing diagnostics are reachable as `14bd5427` and pass the owning repository's release gates. | Retain these conformance gates in the cross-stack baseline. |
...
| Grust | Durable cognition scheduler/store, leases, guarded commit/recovery, ID-only outbox, typed no-change outcome, and Sail executor are reachable as `3bbd715`; the bounded Sail integration harness is reachable as `e20d69c`. The QueryGraph Sail baseline tracks graph revision `c5309365`, which carries upstream performance head `ce5dada0`. Its exact production binary passes 26 adapter tests, the non-null and temp-view checks, and both governed cognition parity/secrecy tests. | Retain the bounded gate in the release matrix. |
...
| Sail | The verified baseline is exact reachable QueryGraph graph revision `c5309365`, recorded in `compat/sail-revision.txt`. It carries the open draft upstream performance PR [lakehq/sail#2400](https://github.com/lakehq/sail/pull/2400) at `ce5dada0` without claiming that PR is merged. The source-verified, native-CPU, `-O3`, thin-LTO, one-codegen-unit, stripped production binary passes the bounded live gate. | Retain the exact graph revision and binary digest in release artifacts. |
