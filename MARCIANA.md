...
| Marciana | `querygraph-memory` has been transplanted with preserved Git history into `crates/marciana-memory`; its Grust and TypeSec dependencies now use exact reachable Git revisions. The native `marciana-catalog` crate owns LakeCat proof-to-cognition-source translation and the cognition-specific catalog/table-identity validation; `marciana-cognition` owns the closed fixed-profile engine binding. The durable scheduler returns staged work only as its proposal digest, forbidding post-restart re-planning; the full unit/integration suite and strict Clippy checks pass against those revisions. | Move the remaining governed `improve` composition into native Marciana crates. |
