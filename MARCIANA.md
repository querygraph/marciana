...
| Grust | Durable cognition scheduler/store, leases, guarded commit/recovery, ID-only outbox, typed no-change outcome, and Sail executor are reachable as `3bbd715`; the bounded Sail integration harness is reachable as `e20d69c`. The QueryGraph Sail baseline now tracks upstream merge `2c1b2e45` and retains the bounded executor gate. | Retain the bounded gate in the release matrix. |
...
| Sail | The supported production baseline is upstream Sail merge `2c1b2e45` ([lakehq/sail#2374](https://github.com/lakehq/sail/pull/2374)), recorded in `compat/sail-revision.txt`; its explicit binary remains subject to the bounded live gate. | Run the refreshed baseline gate and retain the exact upstream revision in release artifacts. |
