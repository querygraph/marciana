...
| Grust | Durable cognition scheduler/store, leases, guarded commit/recovery, ID-only outbox, typed no-change outcome, and Sail executor are committed as `0aca778`; formatting and compile/test gates pass. On 2026-08-06 the explicit local Sail binary also passed 26 `grust-sail` tests, 2 live backend tests, and 2 live cognition parity/secrecy tests. | Select remotely reachable Grust and Sail revisions, then rerun this gate against the canonical refreshed Sail source. |
| QueryGraph | TypeDID/LakeCat cognition boundary, fixed host engine binding, no-change application, opaque proof adaptation, and category-safe errors are committed as `2abc997`; qg-rust now resolves the standalone Marciana path in `f25863b`, and 100 tests plus doctests pass. | Replace the development path with a reachable Marciana revision before clean-clone release gates. |
...
| Marciana | `querygraph-memory` has been transplanted with preserved Git history into `crates/marciana-memory`; qg-rust consumes it from the standalone path. The durable scheduler now returns staged work only as its proposal digest, forbidding post-restart re-planning; focused recovery tests and strict Clippy pass. | Replace temporary sibling paths with reachable revisions, complete the four-verb adapters, and pass the cross-stack gates. |
